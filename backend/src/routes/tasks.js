...
router.post('/', requireAdmin, async (req, res) => {
    ...
    await logActivity(
        ...
        req.user.id,
        'TASK_CREATED',
        `Administrator ${req.user.name} created task: ${task.title}`,
        { taskId: task.id }
    );
...
router.post('/batch', requireAdmin, async (req, res) => {
    ...
    await logActivity(
        ...
        req.user.id,
        'TASK_BATCH_CREATED',
        `Administrator ${req.user.name} created ${createdTasks.length} tasks in batch`,
...
router.post('/:id/accept', async (req, res) => {
    ...
    // LOG ACTIVITY
    await logActivity(
        updatedTask.companyId,
        req.user.id,
        'TASK_ACCEPTED',
        `Driver ${req.user.name} accepted task: ${updatedTask.title}`,
...
router.post('/:id/reject', async (req, res) => {
    ...
    await logActivity(
        ...
        req.user.id,
        'TASK_REJECTED',
        `Driver ${req.user.name} rejected task: ${updatedTask.title}`,
        { taskId: updatedTask.id }
    );
    // ...
});
...
router.post('/:id/cancel', async (req, res) => {
    ...
    await logActivity(
        ...
        req.user.id,
        'TASK_CANCELLED',
        `Driver ${req.user.name} cancelled task: ${updatedTask.title}`,
        { taskId: updatedTask.id }
    );
    // ...
});
...
router.post('/:id/complete', async (req, res) => {
    ...
    await logActivity(
        ...
        req.user.id,
        'TASK_COMPLETED',
        `Driver ${req.user.name} completed task: ${updatedTask.title}`,
        { taskId: updatedTask.id, price: updatedTask.price }
    );
    // ...
});
